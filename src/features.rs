...
impl VKFeatures {
    pub fn load(proto: &ViaKeychronProtocol) -> ViaResult<Self> {
        proto.get_support_features()
    }
}

pub trait VKFeaturesTrait {
    fn get_support_features(&self) -> ViaResult<VKFeatures>;
}

impl VKFeaturesTrait for ViaKeychronProtocol<'_> {
    fn get_support_features(&self) -> ViaResult<VKFeatures> {
        ...
        Ok(VKFeatures::from_bits_retain(u16::from_le_bytes(features)))
    }
}
